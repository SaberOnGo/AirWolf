...
void userInit(void)
{
        os_memset((uint8_t*)&currentDataPoint, 0xFF, sizeof(dataPoint_t));
...
{
    currentDataPoint.valuetime_stamp = RTCDrv_CalendarToSec(1970, &calendar) + 16L * 3600L;
    if(tSDRR.humi == 0xFF){ currentDataPoint.valuehumi = UINT16_MAX; }
    else {  currentDataPoint.valuehumi =   tSDRR.humi;  }                      //Add Sensor Data Collection
    ...
    if(tSDRR.temp != UINT16_MAX)
               currentDataPoint.valuetemp = tSDRR.temp;                                   //Add Sensor Data Collection
...
void TimerWiFiRoutine_CallBack(void * arg)
{
      ...
      if(cnt > 10)
      {
               ...
               recv_timeout_cnt++;
               sec_cnt++;
               if( sec_cnt > 10)  // 每 10 sec 查询一次
               {
                      sec_cnt = 0;
                      GIZWITS_LOG("query wifi  sec = %ld \r\n",  os_get_tick() / 100);
                      gizwitsGetModuleInfo();
               }
               if(recv_timeout_cnt > 30)
               {
                        GIZWITS_LOG("WIFI Disconnect\r\n");
                        GIZWITS_LOG("WiFi No Resp\r\n");
                        
                        SnsGUI_DisplayWifiTip(TIP_WIFI_NoResp);
                        WIFI_SetConnectStatus(NET_STA_NoResp);
...
// notify AP open
//static os_timer_t tTimerAPOpenTip;
//static void TimerAPOpenTip(void * arg)
//{
 //     SnsGUI_DisplayWifiTip(TIP_WIFI_Erase);
//}
     ...
     switch(key_state)
	{
	        case L_key:  
		{
			...
			os_timer_setfn(&tTimerEraseWifiTip, TimerEraseWifiTip,  NULL);
                     os_timer_arm(&tTimerEraseWifiTip,  SEC(50),  0);
			 //wifi_tip_type = TIP_WIFI_TryCon;
			 
#if GIZWITS_TYPE == GIZ_MCU			
			gizwitsSetMode(WIFI_AIRKISS_MODE);
			gizwitsSetMode(WIFI_AIRKISS_MODE);
#elif GIZWITS_TYPE == GIZ_SOC
...
void key1_hook(uint8_t key_state)
{
     switch(key_state)
	{
	        case L_key:  
		{
			...
			//KEY_DEBUG("wifi AP open\r\n");  
			//SnsGUI_DisplayWifiTip(TIP_WIFI_AP_OPEN);
			//os_timer_setfn(&tTimerAPOpenTip,   TimerAPOpenTip,   NULL);
			//os_timer_arm(&tTimerAPOpenTip,   SEC(50),   0);
#if GIZWITS_TYPE == GIZ_MCU	
