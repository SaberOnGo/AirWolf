...
void SnsGUI_ChangeHCHOColor(UG_COLOR c)
{
       // HCHO 字符
...
void SnsGUI_DisplayWifiTip(WifiTipType TipType)
{
      ...
      switch(TipType)
      {
              case TIP_WIFI_OK:
              {
                      if(NetModuleRead() == NET_SEL_WIFI)
                      {
                              UG_PutString(x, y,   "  WIFI OK   ");
                      }
                      else
                      {
                             UG_PutString(x, y,   "  GPRS OK   ");
                      }
              }break;
              ...
              {
                     UG_PutString(x, y,    "Net TryConn");
              }break;
              case  TIP_WIFI_OnConn:
              {
                     UG_PutString(x, y,    "Net On Con ");
              }break;
              case TIP_WIFI_Reset:
              {
                     UG_PutString(x, y,    " Net Reset ");
              }break;
              case TIP_WIFI_Failed:
              {
                     UG_PutString(x, y,    " Net  Failed ");
              }break;
              case TIP_WIFI_Erase:
              {
                      UG_PutString(x, y,    "            ");
              }break;
              case TIP_WIFI_ReadyConn:
              {
                     UG_PutString(x, y,    " Net Conn?  ");
              }break;
              case TIP_WIFI_ReadyReset:
              {
                    UG_PutString(x, y,    " Net  Reset ?");
              }break;
              case TIP_WIFI_AP_OPEN:
              {
                   UG_PutString(x, y,  "AP  Open");
              }break;
              case TIP_WIFI_NoResp:
              {
                     UG_PutString(x, y,  "Net No Resp");
              }break;
              default:
...
void NoneClearUI(void){}

// CO2 显示清除函数
void CO2ClearUI(void)
{
        // 清除CO2 字符
        UG_FontSelect(&FONT_12X20);  // 字符宽 12 pixel, 高为 20 pixel
        UG_SetBackcolor(C_WHITE);
        UG_SetForecolor(C_WHITE);
        UG_PutString(circle[CIRCLE_CO2].x0 - 12,  circle[CIRCLE_CO2].y0 - 13 - 20,   "   ");
        // 清除单位
        UG_FontSelect(&FONT_10X16);  
        UG_PutString(circle[CIRCLE_CO2].x0 - 15,  circle[CIRCLE_CO2].y0 + 13 , "   ");
}

void HCHOClearUI(void)
{
        // 清除 HCHO 字符
       UG_FontSelect(&FONT_10X16);  
       UG_SetBackcolor(C_WHITE);
	UG_SetForecolor(C_WHITE);
	UG_PutString(circle[CIRCLE_HCHO].x0 - 10 * 2, 
	                                circle[CIRCLE_HCHO].y0 - 13 - 16,   "    ");
	// HCHO 单位: ug/m3
	UG_PutString(circle[CIRCLE_HCHO].x0 - 15,  circle[CIRCLE_HCHO].y0 - 13 + 16 + 10 , "     ");
}

// 清除函数, 用于清除单个传感器的界面显示
typedef void (*SnsClearUI)(void);

SnsClearUI snsClearUIFunc[] = {
   NoneClearUI,       // PM10, 左上角
   NoneClearUI,      // TVOC, 左下角 
   NoneClearUI,      // PM2.5, 中间
   HCHOClearUI,     // HCHO, 右上角
   CO2ClearUI,        // CO2, 右下角
};
...
void SnsGUI_DisplayVersion(char * version, char * date, char * time)
{
	...
	UG_SetForecolor(C_BLACK);
	UG_PutString(112, 85,       "AIR WOLF");
	UG_PutString(82, 110,       version);
	UG_PutString(82, 135,     date);
...
#define  C_TIMEOUT_TO_STOP   2  // 超时停止转圈
...
void Timer_TurnCircle_CallBack(void * arg)
{
   ...
   for(j = 0; j < CIRCLE_SIZE; j++)
   {
         ...
         {
                if((os_get_tick() / 100) < 30)  // 开机后 30秒内
                {
		  ...
		  {
		           u8 m;
                         snsTurn[(u8)j][IDX_STATUS] = C_TIMEOUT_TO_STOP;
                         for(m = 0; m< 8; m++)  // 清除弧段
                         {
                                drawSnsArcFunc[(u8)j](m, C_WHITE);
                         }
                         // 清除字符, 单位等
                         snsClearUIFunc[j]();
		  }
