   ...
   if(res == FR_OK)
   {
	  ...
	  if(res == FR_OK)
	  {
         ...
         if(! StringToInt(buf, "gap=", 4, 0, 600, &p, &n))
         {
          ...
          // 读取网络模块
         if(! StringToInt(buf,   (char *)"net_sel=",  8,  0, 10, &p, &n))
         {
                E_NET_SEL net = NET_SEL_WIFI;
                
                if(n)
              {
                    cfgVar_net_sel  = n;   
                     fat_printf("cfgVar_net_sel = %d\r\n",  cfgVar_net_sel);
               }
	   ...
	   do
	  {
			 ...
			 if(n == 1) // n = 1: 只执行一次设置
			 {
			    ...
			    if(n) {  cfgVar_DiskFreeSize  = n;  }
                 ...
                 // 是否获取mac地址
                 if(! StringToInt(buf, "get_mac=", 8, 0, 10, &p, &n))
                 {
			    if(n) {  cfgVar_GetMac = n;  }
                 }
	     }while(0);
	}while(0); 
