...
void key2_hook(uint8_t key_state)
{
     switch(key_state)
	{
	        case S_key:  
...
// 电源键处理
void key_pwr_process(void)
{
               static uint32_t key_time = 0;
               
               if( KeyDect_State() == 0 )   // 电源键被按下, 需长按 3 秒
               {
                       if(key_time == 0)
                       {
                              key_time = os_get_tick() + 300;
                              KEY_DEBUG("key pwr set, tick = %ld\r\n", os_get_tick());
                       }
                       else if(KeyDect_State() == 0)
                       {
                               if(os_get_tick() > key_time)
                               {
                                          KEY_DEBUG("key enter %ld \r\n",  os_get_tick());
                                          UserGUI_LCDClear(C_BLACK);
                                          UG_FontSelect(&FONT_10X16);  
                                          UG_SetBackcolor(C_BLACK);
                                          UG_SetForecolor(C_WHITE);
                                          UG_PutString(125, 100,       "Closing");
                                          UG_PutString(15,      130,       "Pls Release The Power Key");
               ...
               {
                      KEY_DEBUG("set key time 0, tick = %ld \r\n",  os_get_tick());
                      key_time = 0;
               }
 }
